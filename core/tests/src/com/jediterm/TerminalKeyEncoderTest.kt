...
class TerminalKeyEncoderTest {
  ...
  @Test
  fun `Alt Backspace`() {
    assertKeyCode(Ascii.BS.toInt(), InputEvent.ALT_DOWN_MASK, byteArrayOf(Ascii.ESC, Ascii.DEL))
  }

  @Test
  fun `Alt Left`() {
    val expected = if (Platform.isMacOS()) prependEsc("b") else prependEsc("[1;3D")
    assertKeyCode(KeyEvent.VK_LEFT, InputEvent.ALT_DOWN_MASK, expected)
  }

  @Test
  fun `Shift Left`() {
    assertKeyCode(KeyEvent.VK_LEFT, InputEvent.SHIFT_DOWN_MASK, prependEsc("[1;2D"))
  }
  ...
  @Test
  fun `Control F1`() {
    assertKeyCode(KeyEvent.VK_F1, InputEvent.CTRL_DOWN_MASK, prependEsc("[1;5P"))
  }

  @Test
  fun `Control F11`() {
    assertKeyCode(KeyEvent.VK_F11, InputEvent.CTRL_DOWN_MASK, prependEsc("[23;5~"))
  }
